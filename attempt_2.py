...
url = "https://www.wesis.org/sign_in"
driver.get(url)
time.sleep(1)
...
for number in range(1, 10):
    try:
        driver.get(f"https://www.wesis.org/indicators/{number}")
        logs = driver.get_log('browser')[0].get('message')
        if '404' in logs:
            write_failed_url(number)
            continue
        ...
        misc_content = driver.find_element(By.XPATH,
                                           "//div[@class='mdc-card info info--active']//div[@class='info-card-content']")
